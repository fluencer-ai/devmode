...
Two enforcements, both keyed off the transcript (deterministic, not vibes):
  ...
  2) ORCHESTRATOR DELEGATION on a full `/devmode` turn (blocks): if THIS turn's
     ...
     `c`/`do`/`wiki`/`lean`/`goal`/`plan`/`adopt`/`start`) and NO `devmode-orchestrator` agent was
...
# `c`, `do`, `wiki` run inline; `lean`/goal/plan/adopt/start are their own (command-defined) flows.
NONFULL = re.compile(r"^(c|do|wiki|lean|goal|plan|adopt|start)\b", re.IGNORECASE)
